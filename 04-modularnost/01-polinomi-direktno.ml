...
let rec drop_while p = function x :: xs when p x -> drop_while p xs | xs -> xs
...
let odvod = function [] -> [] | _ :: p -> List.mapi (fun k a -> a * (k + 1)) p
...
let _ =
  let x = [ 0; 1 ] in
  let x_plus_ena = sestej x [ 1 ] in
  potenca x_plus_ena 3 |> odvod |> izpis |> print_endline
