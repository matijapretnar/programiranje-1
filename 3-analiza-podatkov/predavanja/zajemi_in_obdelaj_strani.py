...
def izloci_osebe(niz):
    return [match.groupdict() for match in re.finditer(vzorec_osebe, niz)]


def izloci_podatke_filma(ujemanje_filma):
    film = ujemanje_filma.groupdict()
    film['id'] = int(film['id'])
...
def filmi_na_strani(st_strani):
    ...
    for ujemanje_filma in vzorec_filma.finditer(vsebina):
        yield izloci_podatke_filma(ujemanje_filma)
...
def izloci_gnezdene_podatke(filmi):
    ...
    vloge.sort(key=lambda vloga: (vloga['film'], vloga['oseba'], vloga['vloga']))
...
orodja.zapisi_csv(filmi, ['id', 'naslov', 'dolzina', 'leto', 'ocena', 'opis'], 'obdelani-podatki/filmi.csv')
