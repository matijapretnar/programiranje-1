...
def zajemi_spletne_strani(imenik):
    ...
    parametri = 'sort=num_votes,desc&title_type=feature&count=250'
...
def pocisti_film(film):
    ...
    podatki['metascore'] = int(podatki['metascore'])
    return podatki
...
zajemi_spletne_strani('html-datoteke/')
filmi = izloci_podatke_filmov('html-datoteke/')
filmi, osebe, vloge, zanri = razdeli_tabelo(filmi)
orodja.zapisi_tabelo(filmi, ['id', 'naslov', 'leto', 'dolzina', 'ocena', 'metascore', 'opis'], 'csv-datoteke/filmi.csv')
orodja.zapisi_tabelo(osebe, ['id', 'ime'], 'csv-datoteke/osebe.csv')
orodja.zapisi_tabelo(vloge, ['film', 'oseba', 'vloga'], 'csv-datoteke/vloge.csv')
orodja.zapisi_tabelo(zanri, ['film', 'zanr'], 'csv-datoteke/zanri.csv')
