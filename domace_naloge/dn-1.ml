...
let psi7 (f1, f2) = fun c -> (f1 c, f2 c)
...
let inverz p = 
  let sez_parov = List.mapi (fun i x -> (i, x)) p in
  let sorted = List.sort (fun (_, y1) (_, y2) -> compare y1 y2) sez_parov in
  List.map (fun (x, y) -> x) sorted
...
let rec zgradi_cikel p i cikel =
  if List.mem i cikel then 
    List.rev cikel
  else
    zgradi_cikel p (List.nth p i) (i :: cikel)
...
(* zamenjamo indeksa v identiteti *)
let zamenjaj i j p =
  List.mapi (fun index x -> if index = i then j else if index = j then i else x) p

(* pretvorimo transpozicijo v permutacijo *)
let v_permutacijo dolzina transpozicija =
  let id = List.init dolzina (fun x -> x) in
  let (i, j) = transpozicija in
  zamenjaj i j id

(* kompozitum permutacij *)
let iz_transpozicij dolzina transpozicije = 
  let rec aux kom sez_permutacij =
  match sez_permutacij with
  | [] -> kom
  | per1 :: xs -> aux (kompozitum per1 kom) xs
  in 
  let sez_permutacij = List.map (v_permutacijo dolzina) transpozicije
  and identiteta = List.init dolzina (fun x -> x) in
  aux identiteta sez_permutacij
...
let cikel_v_transpozicije cikel =
  match cikel with
...
let izpis_mreze m =
  let rob = "+" ^ String.make 7 '-' ^ "+" ^ String.make 7 '-' ^ "+" ^ String.make 7 '-' ^ "+" in
  
  let sudoku = Array.mapi 
      (fun i vrstica ->
         let vrstica' = 
           Array.mapi 
             (fun j vrednost ->
         ...
         in
         let koncna_vrstica = "| " ^ String.concat "" (Array.to_list vrstica') ^ "\n" in
         if i = 2 || i = 5 || i = 8 then
           koncna_vrstica ^ rob ^ "\n" 
         else
           koncna_vrstica
  ...
  in
  rob ^ "\n" ^ String.concat "" (Array.to_list sudoku)
...
let ni_v_vrstici (m, vrstica) stevilo =
  let vrstica' = m.(vrstica) in 
  
  let rec aux j =
    if j >= Array.length vrstica' then
      true
    else
      match vrstica'.(j) with
      | None -> aux (j + 1)
      | Some n -> 
          if n = stevilo then
            false
...
let ni_v_stolpcu (m, stolpec) stevilo =
  let rec aux i =
    if i >= Array.length m then
      true
    else
      match m.(i).(stolpec) with
      | None -> aux (i + 1)
      | Some n -> 
          if n = stevilo then
            false
...
let primer_sudoku_400 = ni_v_stolpcu (primer_mreze, 0) 1
(* val primer_sudoku_5 : bool = true *)

let primer_sudoku_401 = ni_v_stolpcu (primer_mreze, 0) 7
(* val primer_sudoku_5 : bool = false *) 
                                      
let ni_v_skatli (m, skatla) stevilo =
  let rec aux' i j =
    if i >= skatla / 3 * 3 + 3 then
      true
    else if j >= skatla mod 3 * 3 + 3 then
      aux' (i + 1) (skatla mod 3 * 3)
    else
      match m.(i).(j) with
      | None -> aux' i (j + 1)
      | Some n -> 
  ...
  in
  aux' (skatla / 3 * 3) (skatla mod 3 * 3)
  
let primer_sudoku_402 = ni_v_skatli (primer_mreze, 0) 7
(* val primer_sudoku_5 : bool = true *) 

let primer_sudoku_403 = ni_v_skatli (primer_mreze, 5) 6
(* val primer_sudoku_5 : bool = false *) 

let primer_sudoku_404 = ni_v_skatli (primer_mreze, 6) 6
(* val primer_sudoku_5 : bool = false *) 

let primer_sudoku_405 = ni_v_skatli (primer_mreze, 8) 2
(* val primer_sudoku_5 : bool = true *) 
