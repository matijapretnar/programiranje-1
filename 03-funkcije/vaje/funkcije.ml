...
let rec mapi f list =
  let rec neki acc lst n =
    match lst with
    |[] -> acc
    |x :: xs -> neki ((f x n):: acc) (xs) (n+1)
  in
  xreverse (neki [] list 0)
...
let rec unzip lst =
  let rec aux og (lst1,lst2) =
    match og with
    |[] -> (xreverse lst1, xreverse lst2)
    |(x,y)::zs -> aux zs ((x::lst1),(y::lst2))
  in
  aux lst ([],[])
